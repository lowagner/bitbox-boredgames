...
{
    FileError ferror = _io_load_instrument(i);
    if (ferror == NoError)
        return NoError;
    instrument_reset();
    return ferror;
...
{
    FileError ferror = _io_load_verse(i);
    if (ferror == NoError)
        return NoError;
    verse_reset();
    return ferror;
...
FileError io_load_anthem(unsigned int i)
{
    FileError ferror = _io_load_anthem(i);
    if (ferror == NoError)
        return NoError;
    anthem_reset();
    return ferror;
